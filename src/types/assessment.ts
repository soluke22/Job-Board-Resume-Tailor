...
const centrality = z.enum(['critical','core','standard','preferred']);
const requirementFields = {kind: z.enum(['hard','preferred','responsibility']), excerpt: text.min(1), centrality: centrality.optional(), centralityExcerpt: text.min(1).optional()};
...
export const extractionSchema = z.object({
  ...
  requirements: z.array(z.object(requirementFields).strict()).min(1).max(100),
}).strict();
...
export const requirementSchema = z.object({id: z.string(), ...requirementFields, start: z.number().int().min(0), end: z.number().int().min(1)}).strict();
