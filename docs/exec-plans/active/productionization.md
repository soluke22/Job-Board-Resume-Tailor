...
Result: company ingestion PASS. Ashby's documented public postings feed does not expose a company-name field, but the exact Fieldguide posting has canonical JobPosting `hiringOrganization.name` and matching page-title metadata. The bounded manual Ashby fallback reads only the exact verified canonical URL through `safeFetchText`; optional provider company fields and existing owner labels are preserved, and missing company stays blank. Focused ATS/discovery tests 10/10, full tests 163/163, Node 24.19.0 typecheck/build/privacy (0 findings)/runtime/release/diff checks PASS. Bounded read-only diff review found no new owner/auth, private-data, AI, or arbitrary-host fetch path. Commit `5b5cbbaee7546591eba49a15cc8804e4078f5aee` was pushed to dev; its Vercel Preview reached Ready and held the stable git-dev alias. URL-only import of a current Fieldguide Ashby engineering posting returned LISTED, Fieldguide, and UNASSESSED in the owner workspace, persisted after reload. The original Fieldguide posting was discovered by board scan and importing that same canonical URL left one JobRecord; no deletion was needed.

Board acceptance PARTIAL / BOARD_DISCOVERY_DEFECT_FOUND: owner-facing validation saved enabled Fieldguide (Ashby), Flo Health (Greenhouse), and Great Gray Group (Lever) sources, all retained after reload. One `/api/discover-jobs` request returned 200 and saved 24 UNASSESSED Fieldguide postings; all remained DISCOVERED with pending artifacts and persisted after reload. The current global `leads.slice(0, 24)` cap consumed the entire verification/save allowance from the first source, so no Greenhouse or Lever posting reached the saved results. The code dispatches one scan to every enabled source, but the UI discards per-source `sourceResults` and Vercel runtime logs expose only the incoming request, not outbound provider outcomes; do not claim independently observed per-provider HTTP status or zero egress from these logs. No Gemini, Brave, or general-search operation exists in this scan/import path. Search Preferences stayed at their pre-existing blank values; no pre-existing jobs or application history were present. The 25 DEV test/discovery records and three board sources remain owner-visible. Next exact step requires separately scoped authorization to fairly allocate the 24-lead cap across sources and surface per-source scan outcomes, then repeat DEV acceptance. No production, Vercel config, Google config, manual Neon, auth, or main change.
...
Result: PASS for the scoped local error-contract and diagnostic patch. The former regression was reproduced first: `503 PROVIDER_UNAVAILABLE` cleared the authenticated private workspace. Under bundled Node 24.19.0, focused client/auth/runtime/security tests pass 37/37 and the full suite passes 159/159; typecheck, build, normal and strict privacy scans (zero findings), runtime, harness, release, and diff whitespace checks pass. Coverage includes the A–E client contract, auth 401/403/503 codes, discovery success/outer-budget/inner-AI-budget/persistence/missing-AI/failure classifications, and PGlite first/repeated/hourly/daily/expired/concurrent/persistence budget behavior. Budget diagnostics are static category-only (`external` or `ai`), with no exception details. Direct non-auth request failures preserve session while session-health/auth-verification outages remain fail-closed. Actual-diff and read-only security review found no blocker/high security regression. The historic production persistence exception remains unavailable because it was swallowed; the current tested transaction/schema is production-compatible, so no unproven provider-budget SQL rewrite or migration was made. Next exact step: obtain categorized evidence from a future recurrence or a separately authorized production-equivalent database diagnostic before claiming BUG-002's original source condition removed; review this local patch first. No commit, deployment, configuration, migration, or live-resource action occurred.
